...
translationHash: 7dae1f066ce3e40d
localized: true
...
Nueve, con soporte de primera clase: Claude Code, Codex, OpenCode, Pi, Antigravity CLI, Grok Build CLI, Qwen, Kimi y GitHub Copilot CLI (Gemini CLI está soportado como wrapper legacy). career-ops es agnóstico respecto a la IA: distribuye archivos de prompts que el CLI ejecuta, así que también puedes apuntarlo a cualquier endpoint compatible con OpenAI o a un modelo local sin cambiar una sola línea de código. Consulta [CLIs de IA compatibles](/docs/supported-clis) para ver la lista actual y cómo invocar cada uno.
