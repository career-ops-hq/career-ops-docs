...
Si **New offers added** es 0, busca primero este aviso en la salida del escaneo:

```text title="Terminal"
⚠️  1 target(s) returned zero jobs without a successful HTTP response: Acme
```

Significa que el portal de esa empresa nunca respondió correctamente, así que ese cero no demuestra que no tenga ofertas. La empresa queda registrada como `unverified_zero` en `data/portal-health.tsv` y cuenta para su racha de fallos. Revisa ese portal antes de cambiar tus filtros.

Si no aparece ese aviso, puede que tu filtro de títulos sea demasiado estricto. Prueba a añadir una palabra clave a tu lista `positive` y vuelve a ejecutar el escaneo.
