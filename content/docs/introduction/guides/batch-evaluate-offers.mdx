...
If a delegated worker finds an agency posting that needs your confirmation, it stops before writing a report, CV, application draft, or tracker row and marks the job `needs_confirmation`. Answer the displayed question in the parent interactive session; that session resumes the held posting only after your explicit answer. `--retry-failed` and `--resume-paused` do not resume confirmation holds.
