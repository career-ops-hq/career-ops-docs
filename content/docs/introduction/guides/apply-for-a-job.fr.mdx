...
icon: FileInput
translationHash: "b9c05c932354420f"
localized: true
...
<Callout title="Avant de commencer">
  Assurez-vous d'avoir terminé le guide [Scanner les portails d'emploi](/fr/docs/introduction/guides/scan-job-portals). La commande `apply` dépend d'un rapport d'évaluation existant. Si aucun rapport n'existe pour le poste, lancez d'abord le pipeline.
</Callout>
...
career-ops trouve le rapport correspondant dans `reports/`, utilise Playwright pour ouvrir une fenêtre de navigateur et lit le formulaire. Il prépare ensuite des réponses pour les champs de texte et de réponse libre pris en charge. Vérifiez les limites de chaque champ avant de coller. Pour Ashby, vous devrez utiliser votre navigateur habituel pour terminer et envoyer la candidature.

<Callout title="Remarque">
  Si Playwright n'est pas disponible, career-ops se rabat sur WebFetch pour lire le formulaire. Consultez [Configurer Playwright](/fr/docs/introduction/guides/set-up-playwright) si vous souhaitez activer l'expérience complète via le navigateur.
</Callout>
...
Le système renvoie une liste numérotée de réponses — une par champ du formulaire — qui suit l'ordre du formulaire. Parcourez la liste de haut en bas. Avant de coller chaque réponse en texte libre, vérifiez les caractères ou mots `utilisés/autorisés` dans le champ réel. `limit unknown` indique que la limite doit être vérifiée dans le formulaire ; si une réponse ne tient pas sans perdre des faits essentiels, révisez-la avant de la coller.
...
<Callout title="Champs d'upload">
  Utilisez le fichier dans `output/` généré lors de la création du rapport. Si aucun PDF n'existe encore, lancez d'abord `/career-ops pdf`, puis revenez au formulaire. Consultez [Générer un PDF](/fr/docs/reference/modes/pdf).
</Callout>

## Envoyer la candidature

Relisez chaque réponse, puis envoyez le formulaire vous-même. Pour Ashby, terminez et envoyez la candidature dans votre navigateur habituel, pas depuis la session Playwright. Vérifiez la page de réussite ou l’e-mail de confirmation avant d’indiquer à career-ops que la candidature a été envoyée.
