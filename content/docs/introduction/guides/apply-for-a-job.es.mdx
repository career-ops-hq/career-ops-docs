...
<Callout title="Antes de empezar">
  Asegúrate de haber completado la guía [Escanear portales de empleo](/es/docs/introduction/guides/scan-job-portals). El comando `apply` depende de un informe de evaluación existente. Si no existe informe para el puesto, ejecuta primero el pipeline.
</Callout>
...
career-ops localiza el informe correspondiente en `reports/`, usa Playwright para abrir una ventana del navegador y lee el formulario. Después redacta borradores para los campos de texto y de respuesta libre compatibles. Comprueba los límites de cada campo antes de pegar. Para Ashby, tendrás que usar tu navegador habitual para terminar y enviar la candidatura.

<Callout title="Nota">
  Si Playwright no está disponible, career-ops recurre a WebFetch para leer el formulario. Consulta [Configurar Playwright](/es/docs/introduction/guides/set-up-playwright) si quieres activar la experiencia completa basada en navegador.
</Callout>
...
El sistema devuelve una lista numerada de respuestas — una por campo del formulario — que refleja el orden del propio formulario. Recorre la lista de arriba abajo. Antes de pegar cada respuesta de texto libre, comprueba `usados/permitidos` caracteres o palabras frente al campo real. `limit unknown` indica que debes verificar el límite en el formulario; si una respuesta no cabe sin perder datos esenciales, revísala primero.
...
<Callout title="Campos de subida de archivos">
  Usa el archivo de `output/` generado al crear el informe. Si todavía no existe ningún PDF, ejecuta primero `/career-ops pdf` y vuelve después al formulario. Consulta [Generar un PDF](/es/docs/reference/modes/pdf).
</Callout>

## Envía la candidatura

Revisa todas las respuestas y envía el formulario tú mismo. Para Ashby, termina y envía en tu navegador habitual, no desde la sesión de Playwright. Comprueba la página de éxito o el correo de confirmación antes de indicar a career-ops que la candidatura se envió.
