...
icon: Globe
translationHash: "8f6714fbb268918b"
localized: true
...
<Callout title="Avant de commencer">
  Terminez d'abord le guide de [démarrage rapide](/docs). Node.js 18 ou une version ultérieure (22.5+ recommandé) doit être installé et vous devez vous trouver dans le dossier racine de career-ops.
</Callout>
...
<Tabs items={["Claude Code", "Codex", "OpenCode", "Qwen CLI"]}>
  <Tab value="Claude Code">
    ```bash
    /career-ops apply {company_name} open the browser with Playwright, inspect the form, and draft answers
    ```
  </Tab>
  <Tab value="Codex">
    ```bash
    Run the /career-ops apply command with the {company_name} argument. Then, use the Playwright MCP to open a browser window, inspect the form, and draft answers live.
    ```
  </Tab>
  <Tab value="OpenCode">
    ```bash
...
Pour Ashby, Playwright peut inspecter le formulaire et aider à rédiger les réponses, mais terminez et envoyez la candidature dans votre navigateur habituel. Vérifiez la page de réussite ou l’e-mail de confirmation avant de la marquer comme envoyée.
