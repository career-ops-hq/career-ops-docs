...
career-ops finds the matching report in `reports/`, uses Playwright to open a browser window, and reads the form. It then generates an answer for every field — ready to copy and paste. For Ashby, you will need your ordinary system browser to finish and submit the application.
...
The system returns a numbered list of answers — one per form field — that mirrors the order of the form. Go through the list top to bottom. For each free-text answer, check `used/allowed` characters or words against the live field before pasting. `limit unknown` means you must verify the limit in the form; if an answer cannot fit without losing essential facts, revise it before pasting.
...
Review every answer, then submit the form yourself. For Ashby, finish and submit in your ordinary system browser, not the Playwright session. Verify a success page or confirmation email before telling career-ops the application was submitted.
