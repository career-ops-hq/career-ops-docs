...
Si un worker délégué rencontre une offre d'agence qui exige votre confirmation, il s'arrête avant d'écrire un rapport, un CV, un brouillon de candidature ou une ligne du tracker, puis marque la tâche `needs_confirmation`. Répondez à la question affichée dans la session interactive parente ; cette session ne reprend la tâche suspendue qu'après votre réponse explicite. `--retry-failed` et `--resume-paused` ne reprennent pas les suspensions en attente de confirmation.
