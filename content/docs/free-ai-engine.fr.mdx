...
Vous utilisez déjà un [CLI pris en charge](/fr/docs/supported-clis) — **Claude Code, OpenCode, Codex, GitHub Copilot CLI, et d'autres** ? C'est réglé — career-ops fonctionne avec tous, et plusieurs proposent une offre gratuite. Ouvrez simplement votre CLI dans le dossier career-ops. Aucune configuration supplémentaire sur cette page.
...
Ensuite, pointez votre CLI d'IA (OpenCode fonctionne bien ici) vers l'endpoint local d'Ollama, ou utilisez l'évaluateur local intégré décrit dans le [guide budget](https://github.com/career-ops-hq/career-ops/blob/main/docs/RUNNING_ON_A_BUDGET.md).
...
Vous voulez les recommandations de modèles maintenues à jour, des exemples par fournisseur, les évaluateurs autonomes (`node openai-eval.mjs`, `node ollama-eval.mjs`) et les flags d'économie de tokens ? Le guide **[Running on a Budget](https://github.com/career-ops-hq/career-ops/blob/main/docs/RUNNING_ON_A_BUDGET.md)** du dépôt principal va en profondeur. career-ops est totalement agnostique du modèle — pointez-le vers n'importe quel endpoint compatible OpenAI sans modifier une ligne de code.
</Callout>
