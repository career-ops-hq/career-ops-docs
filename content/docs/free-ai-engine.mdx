...
Then point your AI CLI (OpenCode works well here) at the local Ollama endpoint, or use the built-in local evaluator described in the [budget guide](https://github.com/career-ops-hq/career-ops/blob/main/docs/RUNNING_ON_A_BUDGET.md).
...
Want the maintained model recommendations, per-provider examples, standalone evaluators (`node openai-eval.mjs`, `node ollama-eval.mjs`), and token-saving flags? The core repo's **[Running on a Budget](https://github.com/career-ops-hq/career-ops/blob/main/docs/RUNNING_ON_A_BUDGET.md)** guide goes deep. career-ops is fully model-agnostic — point it at any OpenAI-compatible endpoint with zero code changes.
</Callout>
