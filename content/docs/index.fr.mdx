...
icon: Rocket
translationHash: 684619f80d0f30d4
localized: true
translatedFrom: /docs
---
...
<div className="rounded-lg bg-radial-[at_bottom] from-blue-500/20 p-4 border bg-origin-border border-fd-primary/10 prose-no-margin dark:bg-black/20">
  <div className="rounded-xl bg-fd-background p-3">
    <Steps>
      ...
      Installez **[Claude Code](https://claude.ai/code)**, **[Codex](https://developers.openai.com/codex/cli)**, **[Qwen Code](https://qwen.ai/qwencode)**, **[OpenCode](https://opencode.ai/)** ou **[GitHub Copilot CLI](https://docs.github.com/en/copilot/github-copilot-in-the-cli)** — l'assistant de codage IA qui fait tourner ce système. career-ops fonctionne avec tous. Connectez-vous avant de continuer.
      </Step>
      <Step>
        ### Node.js 18+ (22.5+ recommandé)

        Installez **[Node.js](https://nodejs.org)** — minimum **18** (cela fonctionne, avec un avertissement), **22.5+ recommandé** (cela active l'index SQLite du tracker). Il exécute les scripts auxiliaires et génère les PDF. Vérifiez votre version avec `node --version`.
      </Step>
      <Step>
        ### Git
...
Ouvrez un terminal dans le dossier où vous voulez installer career-ops. La voie recommandée tient en une seule commande ; une voie avancée avec `git clone` se trouve juste en dessous. Si Node (18+) ou Git manquent, `npx` refusera de s'exécuter — installez-les d'abord (voir les prérequis ci-dessus).
...
<Tabs items={["Claude Code", "Antigravity CLI", "Codex", "Qwen Code", "OpenCode", "GitHub Copilot CLI"]}>
  <Tab value="Claude Code">
    ```bash title="Terminal"
    claude
    ```
  </Tab>
  <Tab value="Antigravity CLI">
    ```bash title="Terminal"
    agy
    ```
  </Tab>
...
<Callout type="warn" title="Vous payez Claude Pro ou Max ? Vérifiez ce que cette session facture">
  Lancez `node doctor.mjs` depuis le dossier `career-ops` avant votre première évaluation. Il indique votre source de facturation en une ligne.

  Si `ANTHROPIC_API_KEY` est définie dans votre environnement, **votre CLI l'utilise et facture au token, même si vous payez Pro ou Max.** Cette précédence est celle de Claude Code, pas celle de career-ops : le doctor se contente de la détecter et de vous le dire. Pour utiliser votre abonnement, supprimez la ligne `export ANTHROPIC_API_KEY=...` de `~/.zshrc`, `~/.bashrc`, `~/.profile` ou du `.env` du projet, redémarrez votre terminal et lancez `/login`. Si vous vouliez utiliser des crédits API, tout va bien.

  Les autres CLI définissent chacun leur propre règle abonnement-contre-clé, que career-ops ne vérifie pas. La réponse complète est dans [la FAQ](/fr/docs/faq).
</Callout>

## Étape 3 : lancer votre première évaluation

Une fois l'onboarding terminé, vous êtes prêt à évaluer une vraie offre d'emploi. Copiez l'URL de n'importe quelle annonce et collez-la dans le chat. Avec Claude Code ou Antigravity CLI, préfixez-la avec la commande explicite ; avec les autres CLI, collez l'URL et demandez à l'agent de lancer l'auto-pipeline :
...
<Tabs items={["Claude Code", "Antigravity CLI", "Codex", "Qwen Code", "OpenCode", "GitHub Copilot CLI"]}>
  ...
  <Tab value="Antigravity CLI">
    ```bash
    /career-ops https://jobs.example.com/senior-engineer-ai
    ```
  </Tab>
  <Tab value="Codex">
    ```bash
    Lancez la commande auto-pipeline avec l'URL de l'offre : https://jobs.example.com/senior-engineer-ai
    ```
  </Tab>
  <Tab value="Qwen Code">
    ```bash
...
Le rapport inclut **Score Evidence** après Risk Summary. Cette section couvre cinq dimensions : adéquation du CV, alignement avec North Star, rémunération, signaux culturels et signaux d'alerte. Chaque ligne indique un état de preuve (`supported`, `partial` ou `unknown`), une source ou observation concrète et une question non résolue. La section se termine par un niveau de confiance dans les preuves (`High`, `Medium` ou `Low`), sa justification et jusqu'à trois priorités de vérification.
...
- **Score** — un nombre de 1 à 5. En dessous de 4.0, l'offre n'est probablement pas très adaptée à votre profil.
...
<Accordions>
  ...
  <Accordion title="Le PDF ne s'est pas généré">
    Relancez `npx playwright install chromium`. Si cela ne règle pas le problème, vérifiez que Node.js est en version 18 ou supérieure (22.5+ recommandé) en exécutant `node --version`.
  </Accordion>
...
- **Signaler un bug :** ouvrez une issue sur le [dépôt GitHub](https://github.com/career-ops-hq/career-ops/issues). Décrivez ce que vous attendiez, ce qui s'est réellement passé, et collez tout message d'erreur que vous avez vu.
...
- **Contribuer un correctif :** si vous savez corriger le problème vous-même, lisez [CONTRIBUTING.md](https://github.com/career-ops-hq/career-ops/blob/main/CONTRIBUTING.md) avant d'ouvrir une pull request. En résumé : ouvrez d'abord une issue pour discuter du changement, puis soumettez votre PR.