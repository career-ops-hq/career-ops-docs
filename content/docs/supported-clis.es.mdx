...
career-ops ofrece soporte de primer nivel para **Claude Code, Codex, OpenCode, Pi, Antigravity CLI, Grok Build CLI, Qwen, Kimi y GitHub Copilot CLI** — nueve CLIs de programación con IA con mantenimiento activo. (Gemini CLI se mantiene como wrapper legacy tras su transición a Antigravity CLI.) La arquitectura es compartida: un único `AGENTS.md` canónico dirige la lógica, y unos archivos de entrada ligeros por CLI resuelven los matices de invocación de cada herramienta.
...
[Pi](https://github.com/earendil-works/pi) no necesita archivo wrapper: carga `AGENTS.md` desde el directorio de trabajo y descubre automáticamente la skill compartida en `.agents/skills/career-ops/SKILL.md`, que expone como el comando `/skill:career-ops`. Instálalo con `npm install -g --ignore-scripts @earendil-works/pi-coding-agent`. Para ejecuciones headless/batch usa `pi -p "prompt"`. Pi pide una decisión de confianza por carpeta, una sola vez, antes de leer los recursos del proyecto — confírmala en modo interactivo con `/trust` (se guarda para futuras ejecuciones) o pasa `-a`/`--approve` para confiar solo en esta ejecución.
