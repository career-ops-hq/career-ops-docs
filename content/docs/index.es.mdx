...
<div className="rounded-lg bg-radial-[at_bottom] from-blue-500/20 p-4 border bg-origin-border border-fd-primary/10 prose-no-margin dark:bg-black/20">
  <div className="rounded-xl bg-fd-background p-3">
    <Steps>
      ...
      Instala **[Claude Code](https://claude.ai/code)**, **[Codex](https://developers.openai.com/codex/cli)**, **[Qwen Code](https://qwen.ai/qwencode)**, **[OpenCode](https://opencode.ai/)** o **[GitHub Copilot CLI](https://docs.github.com/en/copilot/github-copilot-in-the-cli)** — el asistente de programación con IA que ejecuta este sistema. career-ops funciona con todos ellos. Inicia sesión antes de continuar.
      </Step>
      <Step>
        ### Node.js 18+ (22.5+ recomendado)

        Instala **[Node.js](https://nodejs.org)** — mínimo **18** (funciona, con una advertencia), **22.5+ recomendado** (habilita el índice SQLite del tracker). Ejecuta los scripts auxiliares y genera los PDFs. Comprueba tu versión con `node --version`.
      </Step>
      <Step>
        ### Git
...
Abre una terminal en la carpeta donde quieras que viva career-ops. La ruta recomendada usa un solo comando; debajo hay una ruta avanzada con `git clone`. Si te faltan Node (18+) o Git, `npx` se negará a ejecutarse — instala ambos primero (ver prerrequisitos arriba).
...
<Callout type="warn" title="¿Pagas Claude Pro o Max? Comprueba qué está facturando esta sesión">
  Ejecuta `node doctor.mjs` desde la carpeta `career-ops` antes de tu primera evaluación. Te informa de tu fuente de facturación en una línea.

  Si `ANTHROPIC_API_KEY` está en tu entorno, **tu CLI la usa y cobra por token, aunque estés pagando Pro o Max.** Esa precedencia es de Claude Code, no de career-ops: el doctor solo la detecta y te lo dice. Para usar tu suscripción, quita la línea `export ANTHROPIC_API_KEY=...` de `~/.zshrc`, `~/.bashrc`, `~/.profile` o del `.env` del proyecto, reinicia la terminal y ejecuta `/login`. Si querías usar créditos de la API, no pasa nada.

  Los demás CLIs fijan cada uno su propia regla de suscripción frente a clave, que career-ops no comprueba. Tienes la respuesta completa en [las preguntas frecuentes](/es/docs/faq).
</Callout>

## Paso 3: Ejecuta tu primera evaluación

Una vez que el agente te haya hecho el onboarding, estás listo para evaluar una oferta real. Copia la URL de cualquier anuncio de empleo y pégala en el chat. Con Claude Code o Antigravity CLI, ponle delante el comando explícito; con los demás CLIs, pega la URL y pídele al agente que ejecute el auto-pipeline:
...
El informe incluye **Score Evidence** después de Risk Summary. Abarca cinco dimensiones: ajuste del CV, alineación con North Star, remuneración, señales culturales y señales de alerta. Cada fila recoge un estado de evidencia (`supported`, `partial` o `unknown`), una fuente u observación concreta y una pregunta pendiente. La sección termina con un nivel de confianza en la evidencia (`High`, `Medium` o `Low`), su motivo y hasta tres prioridades de verificación.

Abre el archivo de informe en `reports/`. El nombre sigue el patrón `{número}-{empresa}-{fecha}.md`. Dentro encontrarás:

- **Nota** — un número del 1 al 5. Por debajo de 4.0 significa que el puesto probablemente no encaja bien.
...
<Accordions>
  ...
  <Accordion title="El PDF no se generó">
    Ejecuta `npx playwright install chromium` de nuevo. Si eso no lo arregla, comprueba que Node.js es la versión 18 o superior (22.5+ recomendado) ejecutando `node --version`.
  </Accordion>
...
- **Reportar un fallo:** Abre un issue en el [repositorio de GitHub](https://github.com/career-ops-hq/career-ops/issues). Describe qué esperabas que pasara, qué pasó en realidad y pega cualquier mensaje de error que vieras.
...
- **Contribuir un arreglo:** Si sabes cómo arreglar el problema tú mismo, lee [CONTRIBUTING.md](https://github.com/career-ops-hq/career-ops/blob/main/CONTRIBUTING.md) antes de abrir un pull request. La versión corta: abre primero un issue para comentar el cambio y luego envía tu PR.
