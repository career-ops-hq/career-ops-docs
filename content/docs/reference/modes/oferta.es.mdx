...
Este nivel describe la evidencia que respalda el Global Score; no es una probabilidad de contratación ni el nivel de legitimidad del anuncio del bloque G, y no modifica la puntuación numérica. Machine Summary conserva los cinco estados en `score_evidence`, el nivel en `confidence` y las prioridades en `confidence_gaps`. Si no quedan prioridades, usa `confidence_gaps: []`.
