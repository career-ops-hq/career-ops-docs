...
Abre la descripción del puesto y el formulario de candidatura en paralelo. Ejecuta apply cuando el formulario esté cargado. Las respuestas con un límite confirmado muestran `usados/permitidos` caracteres o palabras; comprueba ese estado antes de pegarlas. `limit unknown` indica que no se confirmó el límite: verifícalo en el campo real. Si la respuesta no cabe sin perder datos esenciales, revísala antes de pegarla. El resultado es un borrador, no una candidatura terminada — tú sigues al mando del tono, los datos y cualquier campo extra que el portal añada por encima. apply funciona mejor con Playwright en modo visible, donde lee la página en vivo tal y como tú la ves; sin Playwright, pega las preguntas o comparte una captura de pantalla y redactará a partir de eso.
...
- **Ashby** deduplica candidatos por email dentro de cada empresa, así que una segunda candidatura puede fusionarse silenciosamente con la primera. Si ya existe un informe para esa empresa, apply sugiere un alias de email con `+tag`. Para Ashby, termina y envía la solicitud en tu navegador habitual y comprueba la página de éxito o el correo de confirmación antes de marcarla como enviada.
