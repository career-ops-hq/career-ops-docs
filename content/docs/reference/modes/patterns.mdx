...
Reach for patterns once enough applications have moved past evaluation that outcomes carry signal. The hard gate is five entries with a status beyond "Evaluated" — Applied, Responded, Assessment, Interview, Offer, Rejected, or Discarded; below that the mode exits and asks you to come back with more data. More outcomes mean sharper patterns, so it earns its keep after a month of steady applying.
