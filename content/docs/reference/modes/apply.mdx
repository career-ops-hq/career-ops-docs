...
Open the JD and the application form side-by-side. Run apply once the form is loaded. Answers with a confirmed field limit include `used/allowed` characters or words; check this status before pasting. `limit unknown` means the form limit was not confirmed, so verify it in the live field first. If an answer cannot fit without losing essential facts, revise it before pasting. The output is a draft, not a finished application — you stay in the loop for tone, facts and any field the portal layers on top. apply works best with Playwright in visible mode, where it reads the live page as you see it; without Playwright, paste the questions or share a screenshot and it drafts from that instead.
...
- **Ashby** deduplicates candidates by email per company, so a second application can silently merge into your first. If a report for that company already exists, apply suggests a `+tag` email alias. For Ashby, finish and submit in your ordinary system browser, then verify a success page or confirmation email before marking the application submitted.
