...
Faites appel à patterns une fois qu'un nombre suffisant de candidatures a dépassé le stade de l'évaluation pour que les résultats portent un vrai signal. Le seuil strict est de cinq entrées ayant un statut au-delà de « Evaluated » — Applied, Responded, Assessment, Interview, Offer, Rejected ou Discarded ; en dessous, le mode s'arrête et vous demande de revenir avec davantage de données. Plus vous avez de résultats, plus les schémas sont nets : le mode prend donc toute sa valeur après un mois de candidatures régulières.
