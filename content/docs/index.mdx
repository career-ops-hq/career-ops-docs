...
<Tabs items={["Claude Code", "Codex", "OpenCode", "Qwen Code", "GitHub Copilot CLI", "Antigravity CLI"]}>
...
<Callout type="warn" title="Paying for Claude Pro or Max? Check what this session is billing">
  Run `node doctor.mjs` from the `career-ops` folder before your first evaluation. It reports your billing source in one line.

  If `ANTHROPIC_API_KEY` is set in your environment, **your CLI uses it and bills per token, even while you pay for Pro or Max.** That precedence is Claude Code's, not career-ops': the doctor only detects it and tells you. To use your subscription instead, remove the `export ANTHROPIC_API_KEY=...` line from `~/.zshrc`, `~/.bashrc`, `~/.profile` or a project `.env`, restart your terminal, and run `/login`. If you meant to use API credits, nothing is wrong.

  The other CLIs each set their own subscription-versus-key rule, which career-ops does not check. See [the FAQ](/docs/faq) for the full answer.
</Callout>

## Step 3: Run your first evaluation

Once the agent has onboarded you, you are ready to evaluate a real job posting. Copy the URL of any job ad and paste it into the chat. With Claude Code or Antigravity CLI, prefix it with the explicit command; with the other CLIs, paste the URL and ask the agent to run the auto-pipeline:
...
The report includes **Score Evidence** after Risk Summary. It covers five dimensions: CV match, North Star alignment, compensation, cultural signals, and red flags. Each row records an evidence status (`supported`, `partial`, or `unknown`), a concrete source or observation, and an unresolved question. The section ends with an evidence-confidence tier (`High`, `Medium`, or `Low`), its reason, and up to three verification priorities.

Open the report file in `reports/`. The filename follows the pattern `{number}-{company}-{date}.md`. Inside, you will find:

- **Score** — a number from 1 to 5. Anything below 4.0 means the job is probably not a strong fit.
...
- **Report a bug:** Open an issue on the [GitHub repository](https://github.com/career-ops-hq/career-ops/issues). Describe what you expected to happen, what actually happened, and paste any error message you saw.
...
- **Contribute a fix:** If you know how to fix the issue yourself, read [CONTRIBUTING.md](https://github.com/career-ops-hq/career-ops/blob/main/CONTRIBUTING.md) before opening a pull request. The short version: open an issue first to discuss the change, then submit your PR.
