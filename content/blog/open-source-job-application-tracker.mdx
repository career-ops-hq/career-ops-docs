...
The *schema* is fixed: every tool understands the same nine fields, so the tooling and the dashboard agree on what a row means. The *layout* is not. Columns are read by their header name, not their position, so a tracker that carries a `Location`, `Via` or `URL` column, or one of your own, still reads correctly, with `Score` never shifted into `Status`. Extra columns are supported and kept. A row with the nine canonical columns looks like this:
